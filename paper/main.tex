...
MineWorld~\cite{mineworld} is a visual--action autoregressive world model for the Minecraft environment. Given a set of conditioning frames and a sequence of player actions (keyboard keys and camera deltas), the model predicts future frames. Each frame is tokenized by a VQ-VAE into a $14\times24$ grid of 336 discrete tokens, and the transformer predicts these tokens autoregressively. Figure~\ref{fig:worldmodel} shows a representative sequence of frames produced by the model, illustrating the Minecraft scenes it generates.
